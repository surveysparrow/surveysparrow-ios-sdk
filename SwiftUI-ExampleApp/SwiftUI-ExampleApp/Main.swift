    ...
    var body: some View {
        ZStack {
            NavigationView {
                ...
                }.background(spotCheck.navControllerFinder) // Required: nav listener; dismisses spot check on push/pop
            }.navigationViewStyle(StackNavigationViewStyle()) // Required: stack style so the nav bar stays visible
            spotCheck
        }.onAppear
