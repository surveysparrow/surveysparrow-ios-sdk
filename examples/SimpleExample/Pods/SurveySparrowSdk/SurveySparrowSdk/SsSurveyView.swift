...
@IBDesignable public class SsSurveyView: UIView, WKScriptMessageHandler, WKNavigationDelegate {
  ...
  public func loadFullscreenSurvey(
    parent: UIViewController, delegate: SsSurveyDelegate, domain: String? = nil,
    token: String? = nil, params: [String: String]? = [:]
  ) {
    ...
    if domain != nil && token != nil {
      ...
      validateSurvey(
        domain: domain, token: token, params: params, group: group, completion: completion)
      group.wait()
      if isActive == true {
        parent.present(ssSurveyViewController, animated: true)
      } else {
        ssSurveyViewController.surveyDelegate.handleSurveyValidation(
          response: [
  ...
  public func loadEmbedSurvey(
    domain: String? = nil, token: String? = nil, params: [String: String]? = [:]
  ) {
    ...
    if self.domain != nil && self.token != nil {
      ...
      group.wait()
      if isActive == true {
        ssWebView.isHidden = false
        if params != nil {
          self.params = params ?? [:]
        }
        loadSurvey(domain: domain, token: token)
      } else {
        self.handleSurveyValidation(
          response: [
            "active": String(isActive),
            "reason": reason,
          ] as [String: AnyObject])
  ...
  public func loadSurvey(domain: String? = nil, token: String? = nil) {
    ...
    if self.domain != nil && self.token != nil {
      ...
      urlComponent.path =
        "/\(surveyType == .NPS ? "n" : "s")/ios/\(self.token!.trimmingCharacters(in: CharacterSet.whitespaces))"
